...
class ChannelPruningEnv:
    ...
    def __init__(self, model, checkpoint, data, preserve_ratio, args, n_data_worker=4,
        ...
        # save options
        self.model = model
        self.checkpoint = checkpoint # net.state_dict()
        ...
        self.reward = eval(args.reward) # eval用来执行一个字符串表达式，并返回表达式的值
    ...
    def step(self, action):
        ...
        if self._is_final_layer():
            ...
            reward = self.reward(self, acc, current_flops) # TODO 现在是所有环境都最大化acc，最小化flops；怎么指定约束剪model?

            # if reward > self.best_reward: # TODO 打印最新的压缩结果
    ...
    def reset(self):
        ...
        self.cur_ind = 0 # 游标，表示当前剪枝所在的 layer index
    ...
    def _init_data(self):
        ...
        # for MNIST, split 5k for val
        # for ImageNet, split 3k for val
        # val_size = 5000 if 'cifar' in self.data_type else 3000
        if 'cifar' in self.data_type:
            val_size = 5000
        elif 'MNIST' in self.data_type:
            val_size = 5000
        elif 'imagenet' in self.data_type:
            val_size = 3000

        print(self.data_root) # ./dataset/cifar10
