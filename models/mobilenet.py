...
def conv_bn(inp, oup, stride):
    return nn.Sequential(
        nn.Conv2d(inp, oup, 3, stride, 1, bias=False), # input_channel, output_channel, kernel_size, stride, padding
        nn.BatchNorm2d(oup),
...
def conv_dw(inp, oup, stride):
    return nn.Sequential(
        nn.Conv2d(inp, inp, 3, stride, 1, groups=inp, bias=False), # (in channel, out channel, kernel size, stride, padding, groups)
        nn.BatchNorm2d(inp),
        nn.ReLU(inplace=True),
...
class MobileNet(nn.Module):
    def __init__(self, n_class,  profile='normal'):
        ...
        self.conv1 = conv_bn(3, in_planes, stride=2) # input, output
...
class MobileNetMNIST(nn.Module):
    def __init__(self, n_class,  profile='normal'):
        ...
        self.conv1 = conv_bn(1, in_planes, stride=2) # input, output
